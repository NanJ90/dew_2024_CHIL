...
from data_loaders import Dataset, MedicalDataset, MissDataset, DataLoadersEnum
...
from new_models import DEWClassifier
...
class CustomExperiment():
    ...
    def _run_one_pipeline(self, pipeline, pipeline_name, X_train, y_train, X_test, y_test):
        ...
        errors = np.max(np.clip(y_test - proba_predictions, 0, 1), axis=1)
        # errors = [
    ...
    def do_experiment_one_fold(self, X_train, y_train, X_val, y_val, X_test, y_test):
        ...
        proba_predictions = np.mean(
            np.dstack(list(proba_predictions_per_pipeline.values())),
            axis=-1
        )
