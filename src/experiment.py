...
class CustomClassificationExperiment():
    ...
    def _run_one_pipeline(self, pipeline, pipeline_name, X_train, y_train, X_test, y_test):
        ...
        # errors = np.max(np.clip(y_test_2d - proba_predictions, 0, 1), axis=1)
        ...
        # roc_auc = roc_auc_score(y_test_2d, proba_predictions, multi_class='ovr')
        roc_auc = roc_auc_score(y_test, proba_predictions)
    ...
    def do_kfold_experiments(self):
        ...
        for fold in range(self.dataset.n_folds):
            ...
            # enc = OneHotEncoder()
            # y_train = enc.fit_transform(np.array(y_train)[..., np.newaxis])
            # y_val = enc.fit_transform(np.array(y_val)[..., np.newaxis])
            # y_test = enc.fit_transform(np.array(y_test)[..., np.newaxis])
